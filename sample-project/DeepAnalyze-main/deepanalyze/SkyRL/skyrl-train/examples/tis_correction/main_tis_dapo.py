...
class DAPOTrainer(RayPPOTrainer):
    ...
    def postprocess_generator_output(
        ...
        overlong_buffer_penalty_factor = (
            self.cfg.trainer.algorithm.overlong_buffer.penalty_factor
        )
        # modify rewards here
        ...
        assert not isinstance(
            rewards[0], list
        ), "we assume verifiable sequence level rewards here"
        ...
        max_context_length = (
            self.cfg.generator.max_input_length
        ...
        for i, (prompt_length, response_length) in enumerate(
            zip(prompt_lengths, response_lengths)
        ):
            ...
            if (
                response_length > max_exceed_length
                and response_length <= max_context_length - prompt_length
            ):
                exceed_length = response_length - max_exceed_length
                penalty = (
                    exceed_length / overlong_buffer_len * overlong_buffer_penalty_factor
                )
