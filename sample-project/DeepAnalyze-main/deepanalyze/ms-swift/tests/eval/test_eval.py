...
def test_eval_native():
    ...
    eval_main(
        EvalArguments(
            ...
            extra_eval_args={"stream": True, "ignore_errors": True},
        )
    )
...
def test_eval_url():
    ...
    deploy_args = DeployArguments(
        model="Qwen/Qwen2-VL-7B-Instruct", infer_backend=infer_backend, verbose=False
    )

    with run_deploy(deploy_args, return_url=True) as url:
        eval_main(
            EvalArguments(
                model="Qwen2-VL-7B-Instruct", eval_url=url, eval_dataset=["arc_c"]
            )
        )
