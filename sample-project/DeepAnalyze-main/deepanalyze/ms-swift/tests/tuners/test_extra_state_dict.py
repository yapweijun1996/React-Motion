...
class TestExtraStateDict(unittest.TestCase):
    ...
    def test_swift_extra_state_dict(self):
        ...
        state_dict["classifier.weight"] = (
            torch.ones_like(state_dict["classifier.weight"]) * 2.0
        )
        with open(
            os.path.join(self.tmp_dir, "extra_states", "adapter_model.bin"), "wb"
        ) as f:
        ...
        self.assertTrue(
            torch.allclose(
                state_dict["classifier.weight"], model.base_model.classifier.weight
            )
        )
    ...
    def test_swift_modules_to_save(self):
        ...
        model = Swift.prepare_model(
            model, {"lora1": lora_config, "lora2": lora_config2}
        )
