...
class TestSwift(unittest.TestCase):
    ...
    def lora_injection_with_dtype(self, dtype=torch.float32):
        ...
        self.assertTrue(
            os.path.exists(os.path.join(self.tmp_dir, "default", WEIGHTS_NAME))
        )
        ...
        for key in state_dict:
            ...
            self.assertTrue(
                all(
                    torch.isclose(state_dict[key], state_dict2[key])
                    .flatten()
                    .detach()
                    .cpu()
                )
    ...
    def test_save_to_peft_mix(self):
        ...
        model = Swift.prepare_model(
            model, config={"lora": lora_config, "adapter": adapter_config}
        )
    ...
    def test_save_to_peft_param(self):
        ...
        lora_config = LoRAConfig(
            target_modules=["query", "key", "value"], lora_dtype="float16"
        )
    ...
    def test_save_to_peft_ok(self):
        ...
        model = Swift.prepare_model(
            model, config={"default": lora_config, "lora": lora2_config}
        )
        model.save_pretrained(os.path.join(self.tmp_dir, "original"))
        Swift.save_to_peft_format(
            os.path.join(self.tmp_dir, "original"),
        ...
        model2.load_adapter(
            os.path.join(os.path.join(self.tmp_dir, "converted"), "lora"), "lora"
        )
        state_dict = model.state_dict()
        state_dict2 = {
    ...
    def test_swift_multiple_adapters(self):
        ...
        self.assertTrue(
            os.path.exists(os.path.join(self.tmp_dir, "adapter", WEIGHTS_NAME))
        )
        model2 = Swift.from_pretrained(
    ...
    def test_part(self):
        ...
        model.base_model.encoder.encoder.layer[
            0
        ].attention.self.query._part_part.weight.data = torch.ones_like(
            model.base_model.encoder.encoder.layer[
                0
            ].attention.self.query._part_part.weight.data
        )
        ...
        for name, module in model.named_modules():
            if re.fullmatch(targets, name) and "_part_" not in name:
                ...
                self.assertTrue(
                    model.get_submodule(name + "._part_part").weight.requires_grad
                )
        ...
        self.assertTrue(
            os.path.exists(os.path.join(self.tmp_dir, "part", WEIGHTS_NAME))
        )
        ...
        self.assertTrue(
            all(
                torch.isclose(
                    ...
                    model2.base_model.encoder.encoder.layer[
                        0
                    ].attention.self.query._part_part.weight.data,
                )
                .flatten()
        ...
        self.assertTrue(
            all(torch.isclose(output.logits, output2.logits).flatten().detach().cpu())
        )
        self.assertTrue(
            not all(
                torch.isclose(output_origin.logits, output2.logits)
                .flatten()
                .detach()
                .cpu()
            )
        ...
        self.assertTrue(
            not all(
                torch.isclose(output.logits, output2.logits).flatten().detach().cpu()
            )
        )
        self.assertTrue(
            all(
        ...
        model2.set_active_adapters("part2", offload="meta")
        self.assertTrue(
            not model2.base_model.encoder.encoder.layer[
                0
            ].attention.self.query.base_layer._part_part.activated
        )
        self.assertTrue(
            model2.base_model.encoder.encoder.layer[
                0
            ].attention.self.query.base_layer._part_part2.activated
        )
        ...
        self.assertTrue(
            not model2.base_model.encoder.encoder.layer[
                0
            ].attention.self.query.base_layer._part_part3.activated
        )
        self.assertTrue(
            model2.base_model.encoder.encoder.layer[
                0
            ].attention.self.query.active_adapters
            == ["lora"]
        )
