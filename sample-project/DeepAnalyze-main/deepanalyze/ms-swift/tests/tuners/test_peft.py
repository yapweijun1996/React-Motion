...
class TestPeft(unittest.TestCase):
    ...
    def test_lora_merge(self):
        ...
        def reset_lora_parameters(self, adapter_name, init_lora_weights):
            ...
            if adapter_name in self.lora_A.keys():
                ...
                self.lora_A[adapter_name].weight.data = (
                    self.lora_A[adapter_name].weight.data * ratio
                )
        ...
        self.assertTrue(
            model.base_model.bert.encoder.layer[0].attention.self.key.active_adapter
            == ["test"]
        )
        ...
        for key in state_dict:
            ...
            self.assertTrue(
                all(
                    torch.isclose(state_dict[key], state_dict3[key])
                    .flatten()
                    .detach()
                    .cpu()
                )
    ...
    def test_peft_adalora_injection(self):
        ...
        adalora_config = AdaLoraConfig(
            target_modules=["query", "key", "value"], total_step=1
        )
    ...
    def test_peft_lora_dtype(self):
        ...
        model.save_pretrained(self.tmp_dir, safe_serialization=False)
        self.assertTrue(
            os.path.exists(os.path.join(self.tmp_dir, "additional_config.json"))
        )
        model2 = Swift.from_pretrained(model2, self.tmp_dir)
        self.assertTrue(
            model2.base_model.model.bert.encoder.layer[
                0
            ].attention.self.key.lora_A.default.weight.dtype
            == torch.float16
        )
        ...
        model3 = Swift.from_pretrained(model3, self.tmp_dir)
        self.assertTrue(
            model3.base_model.model.bert.encoder.layer[
                0
            ].attention.self.key.lora_A.default.weight.dtype
            == torch.float32
        )
