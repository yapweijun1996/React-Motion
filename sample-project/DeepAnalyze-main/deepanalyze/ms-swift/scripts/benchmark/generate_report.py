...
class ModelOutput:
    ...
    def tuner_hyper_params(self):
        ...
        if args["sft_type"] == "full":
            if "use_galore" in args and args["use_galore"] == "true":
                hyper_params += (
                    f'galore_rank={args["galore_rank"]}/'
    ...
    def hyper_parameters(self):
        if "learning_rate" not in self.args:
            return ""
        return (
            f'lr={self.args["learning_rate"]}/' f'epoch={self.args["num_train_epochs"]}'
        )
...
def generate_sft_report(outputs: List[ModelOutput]):
    ...
    min_best_metric = 999.0
    min_train_loss = 999.0
    if outputs:
        min_best_metric = min([output.best_metric or 999.0 for output in outputs])
        min_train_loss = min([output.train_loss or 999.0 for output in outputs])

    max_gsm8k = 0.0
    if gsm8k_accs:
        max_gsm8k = max([gsm8k or 0.0 for gsm8k in gsm8k_accs])

    max_arc = 0.0
    if arc_accs:
        max_arc = max([arc or 0.0 for arc in arc_accs])

    max_ceval = 0.0
    if ceval_accs:
        max_ceval = max([ceval or 0.0 for ceval in ceval_accs])

    for output, gsm8k_acc, arc_acc, ceval_acc in zip(
        outputs, gsm8k_accs, arc_accs, ceval_accs
    ):
        ...
        if not is_best_metric:
            ...
            best_metric = (
                "" if not output.best_metric else f"**{output.best_metric:.2f}**"
            )
