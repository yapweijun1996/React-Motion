...
class DatasetMeta:
    ...
    subsets: List[Union[SubsetDataset, str]] = field(
...
def _preprocess_d_info(
    d_info: Dict[str, Any], *, base_dir: Optional[str] = None
) -> Dict[str, Any]:
    ...
    if "messages" in d_info:
        d_info["preprocess_func"] = MessagesPreprocessor(
            **d_info.pop("messages"), columns=columns
        )
    else:
...
def register_dataset_info(
    dataset_info: Union[str, List[str], None] = None,
) -> List[DatasetMeta]:
    ...
    if dataset_info is None:
        dataset_info = os.path.join(
            os.path.dirname(__file__), "data", "dataset_info.json"
        )
