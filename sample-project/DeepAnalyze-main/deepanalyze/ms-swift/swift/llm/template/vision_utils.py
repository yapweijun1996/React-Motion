...
def load_file(path: Union[str, bytes, _T]) -> Union[BytesIO, _T]:
    ...
    if isinstance(path, str):
        ...
        if path.startswith("http"):
            ...
            not path.startswith("data:") and len(path) <= 200
        ):
...
def load_batch(
    path_list: List[Union[str, None, Any, BytesIO]],
    load_func: Callable[[Any], _T] = load_image,
) -> List[_T]:
    res = []
...
def load_video_cogvlm2(video: Union[str, bytes]) -> np.ndarray:
    ...
    end_frame = (
        min(duration, int(clip_end_sec * decord_vr.get_avg_fps()))
...
def load_video_valley(video: Union[str, bytes]):
    ...
    images = [
        transforms.ToPILImage()(image.permute(2, 0, 1)).convert("RGB")
        for image in video
    ]
    return images
