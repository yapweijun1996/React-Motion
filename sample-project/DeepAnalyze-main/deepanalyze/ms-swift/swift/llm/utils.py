...
def find_module_list(model) -> Optional[nn.ModuleList]:
    ...
    for m in model.modules():
        if (
            hasattr(m, "gradient_checkpointing")
            or m.__class__.__name__ == "CheckpointWrapper"
        ):
            return
        if (
...
def _add_gradient_checkpointing(module_list):
    ...
    def _new_forward(self, *args, **kwargs):
        ...
        if new_args is not None and self.gradient_checkpointing and self.training:
            if (
                new_args
                and isinstance(new_args[0], torch.Tensor)
                and requires_grad
                and not new_args[0].requires_grad
            ):
...
def dynamic_gradient_checkpointing(model, including_vit: bool = False) -> None:
    ...
    for tower_name in tower_names:
        ...
        logger.info(
            f"Automatically add gradient_checkpointing to {model_tower.__class__}."
        )
...
def history_to_messages(
    ...
    if not roles:
        ...
        assert len(roles) == len(
            history
        ), f"len(roles): {len(roles)}, len(history): {len(history)}"
...
def save_checkpoint(
    ...
    *,
    ...
    for src_file in (additional_saved_files or []) + [
...
def update_generation_config_eos_token(generation_config, template):
    ...
    for stop_word in stop_words:
        ...
        if (
            isinstance(stop_word, (list, tuple))
            and len(stop_word) == 1
            and stop_word[0] not in eos_token_id
        ):
