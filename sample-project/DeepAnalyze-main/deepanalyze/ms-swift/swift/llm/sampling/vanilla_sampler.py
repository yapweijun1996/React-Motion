...
class VanillaSampler(Sampler):
    ...
    def check_row_valid(rows):
        for row in rows:
            assert not row.get("images") or all(
                [isinstance(img, str) and img for img in row["images"]]
            )
            assert not row.get("videos") or all(
                [isinstance(video, str) and video for video in row["videos"]]
            )
            assert not row.get("audios") or all(
                [isinstance(audio, str) and audio for audio in row["audios"]]
            )
    ...
    def generate(self, data):
        ...
        if len(infer_requests) > 0:
            resp_list = self.infer_engine.infer(
                infer_requests, request_config=request_config
            )
        ...
        for idx, row in enumerate(rows):
            ...
            resps["choices"] = []
            for j in range(
                self.args.num_return_sequences * _cur,
                self.args.num_return_sequences * (_cur + 1),
            ):
    ...
    def do_sample(self, data):
        ...
        for i, resps in enumerate(resp_all):
            ...
            if self.orm_model is None and self.prm_model is None:
                ...
                if self.args.easy_query_threshold is not None and sum(
                    [score > 0 for score in orm_score]
                ) - 1 >= int(
