...
class SwiftInfer(SwiftPipeline):
    ...
    def __init__(self, args: Union[List[str], InferArguments, None] = None) -> None:
        ...
        if args.infer_backend == "vllm" and args.adapters:
            self.infer_kwargs["adapter_request"] = AdapterRequest(
                "_lora", args.adapters[0]
            )
    ...
    def run(self) -> List[Dict[str, Any]]:
        ...
        if args.result_path:
            logger.info(
                f"The inference results have been saved to result_path: `{args.result_path}`."
            )
        return result

    def infer_single(
        self,
        infer_request: Union[InferRequest, Dict[str, Any]],
        request_config: RequestConfig,
    ) -> str:
        res_or_gen = self.infer(
            [infer_request],
            request_config,
    ...
    def _prepare_val_dataset(self) -> HfDataset:
        ...
        val_dataset = sample_dataset(
            val_dataset,
            args.val_dataset_sample,
            args.dataset_shuffle,
            self.random_state,
        )
        return val_dataset
    ...
    def _batch_infer(self, val_dataset, request_config):
        ...
        if not (
            ...
            and rank >= 0
            and args.rank % args.tensor_parallel_size != 0
        ):
