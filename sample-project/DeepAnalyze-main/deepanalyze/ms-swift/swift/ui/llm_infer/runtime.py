...
class Runtime(BaseUI):
    ...
    locale_dict = {
        ...
        "show_log": {
            "value": {"zh": "展示部署状态", "en": "Show running status"},
        },
        "stop_show_log": {
        ...
        "running_tasks": {
            "label": {"zh": "运行中部署", "en": "Running deployments"},
            "info": {
        ...
        "refresh_tasks": {
            "value": {"zh": "找回部署", "en": "Find deployments"},
        },
        "kill_task": {
            "value": {"zh": "杀死部署", "en": "Kill running task"},
        },
    }
        ...
        with gr.Accordion(elem_id="runtime_tab", open=False, visible=True):
            with gr.Blocks():
                with gr.Row(equal_height=True):
                    gr.Dropdown(
                        elem_id="running_tasks", scale=10, allow_custom_value=True
                    )
                ...
                base_tab.element("show_log").click(
                    cls.update_log, [], [cls.element("log")]
                ).then(
                    cls.wait,
    ...
    def get_all_ports(cls):
        ...
        for proc in psutil.process_iter():
            ...
            if any([process_name in cmdline for cmdline in cmdlines]) and any(  # noqa
                [cmd_name == cmdline for cmdline in cmdlines]
            ):  # noqa
                try:
                    ports.add(
                        int(
                            cls.parse_info_from_cmdline(
                                cls.construct_running_task(proc)
                            )[1].get("port", 8000)
                        )
                    )
                except IndexError:
    ...
    def refresh_tasks(cls, running_task=None):
        ...
        for proc in psutil.process_iter():
            try:
                ...
                cmdlines = []
            if (
                any([process_name in cmdline for cmdline in cmdlines])
                and not any([negative_name in cmdline for cmdline in cmdlines])
                and any([cmd_name == cmdline for cmdline in cmdlines])  # noqa
            ):  # noqa
                process.append(cls.construct_running_task(proc))
                if log_file is not None and any(  # noqa
                    [log_file == cmdline for cmdline in cmdlines]
                ):  # noqa
