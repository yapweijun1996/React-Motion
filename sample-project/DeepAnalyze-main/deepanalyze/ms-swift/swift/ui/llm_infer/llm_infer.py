...
class LLMInfer(BaseUI):
    ...
    locale_dict = {
        ...
        "loaded_alert": {"value": {"zh": "模型加载完成", "en": "Model loaded"}},
        "port_alert": {
            "value": {"zh": "该端口已被占用", "en": "The port has been occupied"}
        },
        "chatbot": {
            "value": {"zh": "对话框", "en": "Chat bot"},
        },
        "infer_model_type": {
            "label": {"zh": "LoRA模块", "en": "LoRA module"},
            "info": {
        ...
        "prompt": {
            "label": {"zh": "请输入：", "en": "Input:"},
        },
        "clear_history": {
            "value": {"zh": "清除对话信息", "en": "Clear history"},
        },
        "submit": {
            "value": {"zh": "🚀 发送", "en": "🚀 Send"},
        },
    ...
    def deploy(cls, *args):
        ...
        for key, value in zip(keys, args):
            ...
            compare_value_arg = (
                str(compare_value)
                if not isinstance(compare_value, (list, dict))
                else compare_value
            )
            compare_value_ui = (
        ...
        if os.path.exists(model) and os.path.exists(os.path.join(model, "args.json")):
            ...
            with open(
                os.path.join(kwargs["ckpt_dir"], "args.json"), "r", encoding="utf-8"
            ) as f:
                _json = json.load(f)
        ...
        deploy_args = DeployArguments(
            **{
                key: (
                    value.split(" ")
                    if key in kwargs_is_list and kwargs_is_list[key]
                    else value
                )
        ...
        if sys.platform == "win32":
            ...
            run_command = (
                f"{cuda_param}start /b swift deploy {params} > {log_file} 2>&1"
            )
        else:
            run_command = (
                f"{cuda_param} nohup swift deploy {params} > {log_file} 2>&1 &"
            )
    ...
    def agent_type(cls, response):
        ...
        if response.lower().endswith("observation:"):
            return "react"
        if (
            "observation:" not in response.lower()
            and "action input:" in response.lower()
        ):
            return "toolbench"
        return None
    ...
    def send_message(
        ...
        if system:
            if (
                not infer_request.messages
                or infer_request.messages[0]["role"] != "system"
            ):
        ...
        if media:
            ...
            if not _saved_medias or _saved_medias[-1] != media:
                ...
                infer_request.messages[-1]["content"] = (
                    infer_request.messages[-1]["content"] + f"<{media_type[:-1]}>"
                )
                infer_request.messages[-1]["medias"].append(media)

        if not prompt:
            yield "", cls._replace_tag_with_media(infer_request), gr.update(
                value=None
            ), gr.update(value=None), gr.update(value=None), infer_request
            return
        else:
            infer_request.messages[-1]["content"] = (
                infer_request.messages[-1]["content"] + prompt
            )
        ...
        for chunk in gen_list[0]:
            if chunk is None:
                continue
            stream_resp_with_history += (
                chunk.choices[0].delta.content if chat else chunk.choices[0].text
            )
