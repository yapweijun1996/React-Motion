...
def update_data(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        if builder is not None:
            ...
            if choices:
                choices = [
                    str(choice) if choice is not None else None for choice in choices
                ]
                kwargs["choices"] = choices

        if (
            not isinstance(self, (Tab, TabItem, Accordion))
            and "interactive" not in kwargs
        ):  # noqa
        ...
        if (
            base_builder
            and base_builder.default(elem_id) is not None
            and not kwargs.get("value")
        ):
...
class BaseUI:
    ...
    def valid_elements(cls):
        ...
        for key, value in elements.items():
            if (
                isinstance(value, (Textbox, Dropdown, Slider, Checkbox))
                and key != "train_record"
            ):
    ...
    def valid_element_keys(cls):
        return [
            key
            for key, value in cls.elements().items()
            if isinstance(value, (Textbox, Dropdown, Slider, Checkbox))
            and key != "train_record"
        ]
        ...
        if os.path.exists(local_args_path):
            try:
                if hasattr(arg_cls, "resume_from_checkpoint"):
                    try:
                        args = arg_cls(
                            resume_from_checkpoint=model, load_data_args=True
                        )
            ...
            for key in keys:
                if key not in (
                    ...
                    if key == "ref_model_type":
                        if is_ref_model:
                            values.append(
                                gr.update(value=getattr(model_meta, "model_type"))
                            )
                        else:
                            values.append(gr.update())
