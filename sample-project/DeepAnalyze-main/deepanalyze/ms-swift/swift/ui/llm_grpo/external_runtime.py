...
class RolloutRuntime(Runtime):
    ...
    locale_dict = {
        ...
        "rollout_show_log": {
            "value": {"zh": "展示rollout状态", "en": "Show running status"},
        },
        ...
        "rollout_running_tasks": {
            "label": {"zh": "运行中rollout", "en": "Running rollouts"},
            "info": {
        ...
        "rollout_refresh_tasks": {
            "value": {"zh": "找回rollout", "en": "Find rollout"},
        },
        "rollout_kill_task": {
            "value": {"zh": "杀死rollout", "en": "Kill running task"},
        },
    }
        ...
        with gr.Accordion(elem_id="rollout_runtime_tab", open=False, visible=True):
            with gr.Blocks():
                with gr.Row(equal_height=True):
                    gr.Dropdown(
                        elem_id="rollout_running_tasks",
                        scale=10,
                        allow_custom_value=True,
                    )
                with gr.Row(equal_height=True):
                    gr.Button(
                        elem_id="rollout_refresh_tasks", scale=1, variant="primary"
                    )
                ...
                base_tab.element("rollout_show_log").click(
                    cls.update_log, [], [cls.element("rollout_log")]
                ).then(
                    cls.wait,
                    [base_tab.element("rollout_running_tasks")],
                    [cls.element("rollout_log")],
                    **concurrency_limit,
                )

                base_tab.element("rollout_stop_show_log").click(
                    cls.break_log_event, [cls.element("rollout_running_tasks")], []
                )
