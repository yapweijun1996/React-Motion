...
class LLMRollout(BaseUI):
    ...
    locale_dict = {
        ...
        "data_parallel_size": {
            "label": {"zh": "数据并行大小", "en": "Data parallel size"},
        },
        "max_model_len": {
        ...
        "port": {
            "label": {"zh": "Rollout端口", "en": "Rollout Port"},
        },
        "llm_rollout": {
            "label": {
                "zh": "外部rollout模型部署",
        ...
        "rollout_gpu_id": {
            "label": {"zh": "选择用于rollout的GPU", "en": "Choose GPU for rollout"}
        },
        "more_roll_params": {
            "label": {"zh": "更多rollout参数", "en": "More rollout params"},
            "info": {
    ...
    def do_build_ui(cls, base_tab: Type["BaseUI"]):
        with gr.Accordion(elem_id="llm_rollout", open=False, visible=False):
            ...
            with gr.Blocks():
                with gr.Row():
                    gr.Textbox(
                        elem_id="tensor_parallel_size", lines=1, value="1", scale=4
                    )
                    gr.Textbox(
                        elem_id="data_parallel_size", lines=1, value="1", scale=4
                    )
                    gr.Slider(
                        elem_id="gpu_memory_utilization",
                        minimum=0.0,
                        maximum=1.0,
                        step=0.05,
                        value=0.9,
                ...
                RolloutRuntime.element("rollout_kill_task").click(
                    ...
                    [RolloutRuntime.element("rollout_running_tasks")]
                    + [RolloutRuntime.element("rollout_log")],
                )
    ...
    def rollout(cls, *args):
        ...
        if sys.platform == "win32":
            ...
            run_command = (
                f"{cuda_param}start /b swift rollout {params} > {log_file} 2>&1"
            )
        else:
            run_command = (
                f"{cuda_param} nohup swift rollout {params} > {log_file} 2>&1 &"
            )
