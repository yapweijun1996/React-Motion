...
class LLMSample(BaseUI):
    ...
    locale_dict = {
        ...
        "dataset": {
            "label": {"zh": "数据集名称", "en": "Dataset id/path"},
            "info": {
        ...
        "num_sampling_per_gpu_batch_size": {
            "label": {"zh": "每次采样的批次大小", "en": "The batch size of sampling"}
        },
        "num_sampling_per_gpu_batches": {
            "label": {"zh": "采样批次数量", "en": "Num of Sampling batches"}
        },
        "output_dir": {
            "label": {
                "zh": "存储目录",
        ...
        with gr.TabItem(elem_id="llm_sample", label=""):
            ...
            with gr.Blocks():
                ...
                with gr.Row():
                    ...
                    gr.Slider(
                        elem_id="num_sampling_per_gpu_batch_size",
                        minimum=1,
                        maximum=128,
                        step=1,
                        value=1,
                    ...
                    gr.Slider(
                        elem_id="num_sampling_per_gpu_batches",
                        minimum=1,
                        maximum=128,
                        step=1,
                        value=1,
                ...
                base_tab.element("running_tasks").change(
                    partial(SampleRuntime.task_changed, base_tab=base_tab),
    ...
    def sample(cls, *args):
        ...
        if sys.platform == "win32":
            ...
            run_command = (
                f"{cuda_param}start /b swift sample {params} > {log_file} 2>&1"
            )
        else:
            run_command = (
                f"{cuda_param} nohup swift sample {params} > {log_file} 2>&1 &"
            )
