...
class LLMTrain(BaseUI):
    ...
    locale_dict: Dict[str, Dict] = {
        ...
        "train_stage": {
            "label": {"zh": "训练Stage", "en": "Train Stage"},
            "info": {
                "zh": "请注意选择与此匹配的数据集",
        ...
        "submit": {"value": {"zh": "🚀 开始训练", "en": "🚀 Begin"}},
        "dry_run": {
            "label": {"zh": "仅生成运行命令", "en": "Dry-run"},
            "info": {
                "zh": "仅生成运行命令，开发者自行运行",
        ...
        "gpu_id": {
            "label": {"zh": "选择可用GPU", "en": "Choose GPU"},
            "info": {
        ...
        "train_type": {
            "label": {"zh": "训练方式", "en": "Train type"},
            "info": {"zh": "选择训练的方式", "en": "Select the training type"},
        },
        "seed": {
            "label": {"zh": "随机数种子", "en": "Seed"},
            "info": {"zh": "选择随机数种子", "en": "Select a random seed"},
        },
        "torch_dtype": {
            "label": {"zh": "训练精度", "en": "Training Precision"},
            "info": {"zh": "选择训练精度", "en": "Select the training precision"},
        },
        "envs": {
            "label": {"zh": "环境变量", "en": "Extra env vars"},
        },
        "use_ddp": {
            "label": {"zh": "使用DDP", "en": "Use DDP"},
            "info": {
                "zh": "是否使用数据并行训练",
        ...
        "ddp_num": {
            "label": {"zh": "DDP分片数量", "en": "Number of DDP sharding"},
            "info": {
                "zh": "启用多少进程的数据并行",
        ...
        "use_liger_kernel": {
            "label": {"zh": "使用Liger kernel", "en": "Use Liger kernel"},
            "info": {
        ...
        "resume_only_model_alert": {
            "value": {
                "zh": '检测到"args.json"在{}中，但未检测到优化器参数，将仅加载模型参数开始断点续训',
        ...
        "more_params": {
            "label": {"zh": "其他高级参数", "en": "Other params"},
            "info": {
        ...
        "extra_params": {
            "label": {"zh": "其他参数设置", "en": "Extra settings"},
        },
        "train_param": {
            "label": {"zh": "训练参数设置", "en": "Train settings"},
        },
    }
        ...
        with gr.TabItem(elem_id="llm_train", label=""):
            ...
            with gr.Blocks():
                ...
                with gr.Accordion(elem_id="train_param", open=True):
                    with gr.Row():
                        ...
                        gr.Dropdown(
                            elem_id="train_type",
                            scale=4,
                            choices=list(get_supported_tuners()),
                        )
                ...
                base_tab.element("gpu_id").change(
                    cls.update_ddp_num,
                    [base_tab.element("gpu_id"), base_tab.element("use_ddp")],
                    base_tab.element("ddp_num"),
                )
                ...
                Runtime.element("kill_task").click(
                    ...
                    Runtime.reset,
                    [],
                    [Runtime.element("logging_dir")] + [Hyper.element("output_dir")],
                )
    ...
    def train(cls, *args):
        ...
        for key, value in zip(keys, args):
            compare_value = (
                default_args.get(key)
                if key != "hub_private_repo"
                else extra_default_args.get(key)
            )
            ...
            if (
                key not in ignore_elements
                and key in default_args
                and compare_value != value
                and (value or value in (0, False))
            ):
        ...
        if (
            ...
            and os.path.exists(os.path.join(model, "args.json"))
        ):
            ...
            with open(os.path.join(ckpt_dir, "args.json"), "r", encoding="utf-8") as f:
                ...
                if os.path.exists(os.path.join(ckpt_dir, "scheduler.pt")):
                    ...
                    gr.Info(
                        cls.locale("resume_checkpoint_alert", cls.lang)["value"].format(
                            ckpt_dir
                        )
                    )
                else:
                    ...
                    gr.Info(
                        cls.locale("resume_only_model_alert", cls.lang)["value"].format(
                            ckpt_dir
                        )
                    )
        ...
        try:
            sft_args = RLHFArguments(
                **{
                    key: (
                        value.split(" ")
                        if kwargs_is_list.get(key, False) and isinstance(value, str)
                        else value
                    )
        ...
        params += (
            f"--add_version False --output_dir {sft_args.output_dir} "
        ...
        if model:
            record = {}
            for key, value in zip(keys, args):
                if key in default_args or key in (
                    "more_params",
                    "train_stage",
                    "use_ddp",
                    "ddp_num",
    ...
    def train_local(cls, *args):
        ...
        if cls.group == "llm_grpo" and sft_args.vllm_mode == "server":
            host = (
                sft_args.vllm_server_host if sft_args.vllm_server_host else "127.0.0.1"
            )
        ...
        return (
            run_command,
            sft_args.logging_dir,
            gr.update(open=True),
            Runtime.refresh_tasks(sft_args.output_dir, cls.group),
            gr.update(choices=cls.list_cache(sft_args.model)),
        )
    ...
    def prepare_sub_to_filter(cls):
        tabs_relation_dict = {
            key: val
            for key, val in zip(
                ["train_type", "optimizer", "task_type"],
    ...
    def remove_useless_args(cls, uncleaned_kwargs, tabs_relation_dict):
        for target, tabs_to_filter in tabs_relation_dict.items():
            ...
            for tab_key in tabs_to_filter.keys():
                ...
                if (
                    tab_key == "lisa"
                    and target_value == "full"
                    and uncleaned_kwargs.get("lisa_activated_layers")
                ):
                    continue
                if (
                    tab_key == "lora_ga"
                    and target_value == "lora"
                    and uncleaned_kwargs.get("init_weights") == "lora-ga"
                ):
                    continue
