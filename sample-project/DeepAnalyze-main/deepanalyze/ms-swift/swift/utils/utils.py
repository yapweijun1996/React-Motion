...
def seed_everything(
    seed: Optional[int] = None, full_determinism: bool = False, *, verbose: bool = True
) -> int:
...
def parse_args(
    class_type: Type[_T], argv: Optional[List[str]] = None
) -> Tuple[_T, List[str]]:
...
def subprocess_run(
    command: List[str], env: Optional[Dict[str, str]] = None, stdout=None, stderr=None
):
...
def get_env_args(
    args_name: str, type_func: Callable[[str], _T], default_value: Optional[_T]
) -> Optional[_T]:
    ...
    for pattern in patterns:
        ...
        if len(pattern_parts) > 1:
            ...
            for root, dirs, files in os.walk(source_dir):
                ...
                if rel_path == "." or (
                    rel_path != "." and not fnmatch.fnmatch(rel_path, subdir_pattern)
                ):
                    continue
