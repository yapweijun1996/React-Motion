...
class Part(SwiftAdapter):
    ...
    def prepare_model(model: nn.Module, config: PartConfig, adapter_name: str):
        ...
        for name in name_list:
            ...
            if Part.target_module_matched(name, config) and not getattr(
                module, "plugin", False
            ):
                ...
                def _forward(self, *args, **kwargs):
                    ...
                    sub_modules = [
                        ...
                        if getattr(child, "activated", False)
                    ]
            ...
            for key, value in state_dict.items():
                if f"_part_{adapter_name}." in key:
                    if kwargs.get("replace_key", True):
                        new_key = key.replace(f"_part_{adapter_name}.", "").replace(
                            "base_layer.", ""
                        )
                    else:
                        new_key = key
        ...
        def load_state_dict_callback(
            model: nn.Module, adapter_name: str, state_dict: Dict[str, torch.Tensor]
        ):
            new_state_dict = {}
                    ...
                    for param_name in state_dict:
                        if param_name.startswith(name):
                            ...
                            if "_part_" not in param_name:
                                if hasattr(module, "base_layer"):
                                    new_state_dict[
                                        name + f".base_layer._part_{adapter_name}" + end
                                    ] = state_dict[param_name]
                                else:
                                    new_state_dict[
                                        name + f"._part_{adapter_name}" + end
                                    ] = state_dict[param_name]
                            else:
        ...
        for name in name_list:
            ...
            if re.fullmatch(f".*_part_{adapter_name}$", name):
                ...
                SwiftAdapter.save_memory(
                    sub_module, adapter_name, name, activate, offload
                )
