...
class PromptConfig(SwiftConfig):
    ...
    embedding_pos: Union[str, int] = field(
        default=None, metadata={"help": "The position of the embedding tensor"}
    )
    ...
    attention_mask_value: Union[float, int, bool] = field(
        default=0.0, metadata={"help": "The value to pad to the attention mask"}
    )
    ...
    attach_front: bool = field(
        default=True,
        metadata={
            "help": "When set to True, prompt is attached in front of the embedding"
        },
    )
    ...
    extract_embedding: bool = field(
        default=False,
        metadata={
            "help": "Whether the embedding is extracted at final stage to keep the same dims with inputs"
        },
    )
        ...
        for module_key in module_keys:
            ...
            if target_module_found:  # noqa
                ...
                def _forward(self, *args, **kwargs):
                    ...
                    input_embedding = getattr(self, f"prompt_{adapter_name}").forward(
                        input_embedding
                    )
                    if isinstance(config.embedding_pos, int):
                        args = type(args)(
                            args[0 : config.embedding_pos]
                            + (input_embedding,)
                    ...
                    forward_output = getattr(self, f"forward_origin_{adapter_name}")(
                        *args, **kwargs
                    )
                    if config.extract_embedding:
                        forward_output = getattr(
                            self, f"prompt_{adapter_name}"
                        ).extract(forward_output)
...
class PromptModule(nn.Module, ActivationMixin):
    ...
    def forward(self, x):
        if not self.is_activated(self.adapter_name):
            return x
        prompt_token = self.prompt_token.expand(x.shape[0], -1, -1).to(
            x.device, x.dtype
        )
    ...
    def patch_attention_mask(self, m):
        ...
        prefix_attention_mask = torch.full(
            (*m.shape[:-1], self.prompt_length), self.mask_values
        ).to(m.device)
        if self.attach_front:
