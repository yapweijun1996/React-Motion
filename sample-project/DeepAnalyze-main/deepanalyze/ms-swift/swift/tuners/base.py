...
class SwiftModel(nn.Module):
    ...
    def load_state_dict(self, state_dict, strict=True, adapter_name: str = None):
        if adapter_name is not None:
            ...
            for key, value in copy(state_dict).items():
                ...
                if (
                    f"lora_embedding_A.{adapter_name}." not in key
                    and "lora_embedding_A" in key
                ):
                    state_dict.pop(key, None)
                    key = key.replace(
                ...
                if (
                    f"lora_embedding_B.{adapter_name}." not in key
                    and "lora_embedding_B" in key
                ):
                    state_dict.pop(key, None)
                    key = key.replace(
            ...
            if output.load_state_dict_callback:
                state_dict = output.load_state_dict_callback(
                    self.base_model, adapter_name, state_dict
                )
        ...
        **kwargs,
        ...
        if kwargs.get("save_adapter", True):
            for name, output in self.adapters.items():
                if (
                    adapter_name == name or adapter_name is None
                    ...
                    for module_name in modules_to_save_names:
                        if f"modules_to_save.{name}" in module_name:
                            state_dicts[
                                module_name.replace(f"modules_to_save.{name}.", "")
        ...
        if peft_format:
            ...
            for key, value in state_dicts.items():
                ...
                key = key.replace(
                    f"lora_embedding_A.{adapter_name}.", "lora_embedding_A."
                )
                key = key.replace(
                    f"lora_embedding_B.{adapter_name}.", "lora_embedding_B."
                )
        ...
        **kwargs,
        ...
        if adapter_name is None:
            adapter_name = [
                ...
                if os.path.isfile(os.path.join(model_dir, sub_dir, CONFIG_NAME))
        ...
        for _name in (
            ...
            if isinstance(adapter_name, list)
                ...
                [adapter_name] if isinstance(adapter_name, str) else adapter_name.keys()
            )
        ):
            ...
            if SWIFT_TYPE_KEY not in json_object:
                ...
                key = (
                    _name if not isinstance(adapter_name, dict) else adapter_name[_name]
                )
    ...
    def _prepare_model(
        ...
        if adapter_cls.has_additional_modules() and not getattr(
    ...
    def create_or_update_model_card(self, output_dir: str):
        ...
        if not os.path.exists(os.path.join(output_dir, "README.md")):
            ...
            with open(
                os.path.join(output_dir, "README.md"), "r", encoding="utf-8"
            ) as f:
                lines = f.readlines()
        ...
        if hasattr(self.base_model, "config") and hasattr(
            ...
            if hasattr(self.base_model.config.quantization_config, "to_dict"):
                quantization_config = (
                    self.base_model.config.quantization_config.to_dict()
                )
        ...
        if framework_block_heading in lines:
            lines.insert(
                lines.index(framework_block_heading) + 2, f"- SWIFT {__version__}\n"
            )
        else:
    ...
    def add_weighted_adapter(
        ...
        lora_model.peft_config = {
            key: value.config for key, value in self.adapters.items()
        }
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        if not self.has_additional_modules:
            if hasattr(self.base_model, "save_pretrained"):
                ...
                self._save_state_dict(
                    self.base_model.state_dict(), save_directory, safe_serialization
                )
        ...
        adapter_names = (
            adapter_name
            if isinstance(adapter_name, list) or adapter_name is None
            else [adapter_name]
        )
        ...
        for adapter_name, output in self.adapters.items():
            ...
            output_dir = (
                ...
                if adapter_name != "default" or not save_to_peft
                else save_directory
            )
        ...
        if len(output_state_dict) > 0:
            if self.has_additional_modules:
                os.makedirs(
                    os.path.join(save_directory, self.EXTRA_STATE_DIR), exist_ok=True
                )
                self._save_state_dict(
                    output_state_dict,
                ...
                with open(
                    os.path.join(save_directory, self.EXTRA_STATE_DIR, CONFIG_NAME),
                    "w",
                    encoding="utf-8",
                ) as file:
    ...
    def set_active_adapters(
        self, adapter_names: Union[List[str], str], offload: str = None
    ):
    ...
    def activate_adapter(self, adapter_name: str):
        ...
        SWIFT_MAPPING[self.adapters[adapter_name].config.swift_type][
            1
...
class Swift:
    ...
    def prepare_model(
        model: Union[nn.Module, SwiftModel],
        config: Union[SwiftConfig, PeftConfig, Dict[str, SwiftConfig]],
        **kwargs,
    ):
    ...
    def merge_and_unload(model: Union[PeftModel, SwiftModel], **kwargs):
        ...
        if isinstance(model, _PeftModel):
            ...
            for adapter, output in model.adapters.items():
                if isinstance(output.config, LoRAConfig) and (
                    adapter_name is None or adapter in adapter_name
                ):
        ...
        if not isinstance(model, SwiftModel):
            ...
            if adapter.config.swift_type == SwiftTuners.LLAMAPRO:
                ...
                if is_deepspeed_zero3_enabled():
                    raise ValueError(
                        "DeepSpeed ZeRO3 not supported for LLaMAPro&GRPO currently."
                    )
    ...
    def save_to_peft_format(ckpt_dir: str, output_dir: str) -> None:
        ...
        assert os.path.exists(
            ckpt_dir
        ), f"ckpt_dir: {ckpt_dir} must exists in local disk."
        ...
        adapter_names = [
            sub_dir
            for sub_dir in os.listdir(ckpt_dir)
        ...
        for adapter in adapter_names:
            with open(
                os.path.join(ckpt_dir, adapter, CONFIG_NAME), encoding="utf-8"
            ) as f:
                _json = json.load(f)
        ...
        for adapter in adapter_names:
            ...
            for key, value in state_dict.items():
                ...
                key = key.replace(
                    f"lora_magnitude_vector.{adapter}", "lora_magnitude_vector"
                )
                new_state_dict[key] = value
            state_dict = new_state_dict
            SwiftModel._save_state_dict(
                state_dict, os.path.join(output_dir, adapter), safe_serialization
            )
            from swift import LoRAConfig

            with open(
                os.path.join(output_dir, adapter, CONFIG_NAME), encoding="utf-8"
            ) as f:
                _json = json.load(f)
    ...
    @staticmethod
    def from_pretrained(
        model: Union[nn.Module, SwiftModel, PeftModel],
        model_id: str = None,
        adapter_name: Union[str, List[str], Dict[str, str]] = None,
        revision: str = None,
        **kwargs,
        ...
        _name = (
            adapter_name
            if isinstance(adapter_name, str) or adapter_name is None
            else (
                adapter_name[0]
                if isinstance(adapter_name, list)
                else list(adapter_name.keys())[0]
            )
        )
        _name = _name or ""
        if os.path.exists(os.path.join(model_id, _name, CONFIG_NAME)):
            with open(
                os.path.join(model_id, _name, CONFIG_NAME), "r", encoding="utf-8"
            ) as f:
                _json = json.load(f)
            is_peft_model = (
                SWIFT_TYPE_KEY not in _json and "extra_state_keys" not in _json
            )
        if is_peft_model:
            ...
            def load_peft_model(_model, _adapter_name, _new_name=None):
                ...
                if not isinstance(_model, peft.PeftModel):
                    return PeftModel.from_pretrained(
                        _model,
                        (
                            os.path.join(model_id, _adapter_name)
                            if _adapter_name != "default"
                            and os.path.exists(os.path.join(model_id, _adapter_name))
                            else model_id
                        ),
                        revision=revision,
            ...
            if not adapter_name:
                ...
                for _dir in os.listdir(model_id):
                    if os.path.isdir(os.path.join(model_id, _dir)) and os.path.exists(
                        os.path.join(model_id, _dir, CONFIG_NAME)
                    ):
            ...
            return SwiftModel.from_pretrained(
                model, model_id, revision=revision, adapter_name=adapter_name, **kwargs
            )
