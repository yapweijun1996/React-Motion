...
class LoraConfig(peft.LoraConfig):
    ...
    lorap_lr_ratio: Optional[float] = field(
        default=None, metadata={"help": "The lr ratio of lora_B in lora+"}
    )
    ...
    def save_pretrained(self, save_directory: str, **kwargs) -> None:
        ...
        with open(
            os.path.join(save_directory, "additional_config.json"),
            "w",
            encoding="utf-8",
        ) as f:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if hasattr(PeftConfigMixin, "from_pretrained_origin"):
            ...
            self = super(LoraConfig, cls).from_pretrained(
                pretrained_model_name_or_path, subfolder, **kwargs
            )
...
def _create_and_replace_hook(self, peft_config, adapter_name, target, *args, **kwargs):
    ...
    if (
        ...
        and not any(
            [name in target.__class__.__name__.lower() for name in all_supported_names]
        )
        and not any([isinstance(target, type_) for type_ in all_supported_types])
    ):
        return
    ...
    return self._create_and_replace_origin(
        peft_config, adapter_name, target, *args, **kwargs
    )
...
def _convert_dtype(target: torch.nn.Module, adapter_name: str, lora_dtype: str):
    if lora_dtype is not None:
        ...
        if (
            hasattr(target, "lora_embedding_A")
...
def create_optimizer_param_groups(self: PeftModel, **defaults):
    if (
        not isinstance(self.peft_config[self.active_adapter], LoraConfig)
        or self.peft_config[self.active_adapter].lorap_lr_ratio is None
    ):
        return None
...
def hot_patch_peft_module():
    ...
    def __new_init__(
        self, model: torch.nn.Module, config: Dict[str, LoraConfig], adapter_name: str
    ):
        ...
        for active_adapter in active_adapters:
            active_config = (
                config[active_adapter] if isinstance(config, dict) else config
            )
            if hasattr(active_config, "lora_dtype"):
                for name, module in model.named_modules():
                    if isinstance(module, LoraLayer):
                        _convert_dtype(module, active_adapter, active_config.lora_dtype)
                        for lora in list(module.lora_A.values()) + list(
                            module.lora_B.values()
                        ):
...
def get_wrapped_class(module_class):
    ...
    class PeftWrapper(module_class):
        ...
        @classmethod
        def from_pretrained(
            cls, model, model_id, *args, revision: Optional[str] = None, **kwargs
        ):
