...
class SideConfig(SwiftConfig):
    ...
    target_modules: str = field(
        default=None,
        metadata={"help": "The target module to be replaced, in full match format"},
    )

    side_module_name: str = field(
        default="fcn4", metadata={"help": "The name of the additive side networks"}
    )
...
class Side(SwiftAdapter):
    ...
    @staticmethod
    def prepare_model(
        model: nn.Module, config: SideConfig, adapter_name: str
    ) -> SwiftOutput:
        ...
        for module_key in module_keys:
            if re.fullmatch(config.target_modules, module_key):  # noqa
                ...
                def _forward(self, *args, **kwargs):
                    ...
                    x_main = (
                        args_main[config.target_hidden_pos]
                ...
                side_module = SideModule(
                    config.dim, adapter_name, module_key, config.side_module_name
                )
...
class FCN4(nn.Module):
    ...
    def __init__(self, out_dims=-1, **kwargs):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(
                3, 16, kernel_size=3, stride=1, padding=1, bias=False, dilation=1
            ),
            nn.GroupNorm(2, 16),
            nn.ReLU(),
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(
                16, 16, kernel_size=3, stride=2, padding=0, bias=False, dilation=1
            ),
            nn.GroupNorm(2, 16),
            nn.ReLU(),
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(
                16, 32, kernel_size=3, stride=2, padding=0, bias=False, dilation=1
            ),
            nn.GroupNorm(2, 32),
            nn.ReLU(),
        )
        self.conv4 = nn.Sequential(
            nn.Conv2d(
                32, 64, kernel_size=3, stride=1, padding=0, bias=False, dilation=1
            ),
            nn.GroupNorm(2, 64),
            nn.ReLU(),
        )
