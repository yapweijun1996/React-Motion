...
class SCETuningConfig(SwiftConfig):
    ...
    target_modules: Optional[Union[List[str], str]] = field(
        default=None,
        metadata={
            "help": "The target module to be replaced, can be a regex string or name list of full match format"
        },
    )
    ...
    hint_modules: Optional[Union[List[str], str]] = field(
        default=None,
        metadata={
            "help": "The hint modules to be replaced, can be a regex string or name list of full match format"
        },
    )
    ...
    tuner_mode: str = field(
        default="decoder",
        metadata={
            "help": "Location of tuner operation. The tuner mode choices: encoder, decoder, and identity"
        },
    )

    tuner_op: str = field(
        default="SCEAdapter", metadata={"help": "The tuner ops choices: SCEAdapter"}
    )

    down_ratio: float = field(
        default=1.0, metadata={"help": "The dim down ratio of tuner hidden state"}
    )
...
class SCETuning(SwiftAdapter):
    ...
    @staticmethod
    def prepare_model(
        model: nn.Module, config: SCETuningConfig, adapter_name: str
    ) -> SwiftOutput:
        ...
        if len(hint_module_ins_list) > 0 and not len(hint_module_ins_list) == len(
            target_module_ins_list
        ):
        ...
        for tuner_id, t_module in enumerate(target_module_ins_list):
            setattr(
                t_module, f"forward_origin_{adapter_name}", getattr(t_module, "forward")
            )
