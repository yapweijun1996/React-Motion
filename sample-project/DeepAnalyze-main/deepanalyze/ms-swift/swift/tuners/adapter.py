...
class AdapterConfig(SwiftConfig):
    ...
    target_modules: Union[str, List[str]] = field(
        default=None,
        metadata={
            "help": "The feedforward module to be replaced. in regex format if this argument is str, "
    ...
    adapter_length: int = field(
        default=128,
        metadata={"help": "The length of the adapter length (intermediate length)"},
    )

    act_layer: str = field(
        default="gelu", metadata={"help": "The activation layer of the adapter"}
    )
...
class Adapter(SwiftAdapter):
    ...
    @staticmethod
    def prepare_model(
        model: nn.Module, config: AdapterConfig, adapter_name: str
    ) -> SwiftOutput:
        ...
        for module_key in module_keys:
            ...
            if target_module_found:  # noqa
                ...
                def _forward(self, *args, **kwargs):
                    args = getattr(self, f"forward_origin_{adapter_name}")(
                        *args, **kwargs
                    )
                ...
                num_args_in_forward_chunk_fn = len(
                    inspect.signature(
                        getattr(module, f"forward_origin_{adapter_name}")
                    ).parameters
                )
                if (
                    config.method_name == "feed_forward_chunk"
                    and num_args_in_forward_chunk_fn == 1
                ):
                    setattr(
                        module,
                        config.method_name,
                    ...
                    setattr(
                        module, config.method_name, types.MethodType(_forward, module)
                    )
