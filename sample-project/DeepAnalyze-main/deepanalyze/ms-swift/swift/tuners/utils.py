        ...
        if os.path.isfile(os.path.join(pretrained_model_name_or_path, CONFIG_NAME)):
            ...
            try:
                model_dir = snapshot_download(
                    pretrained_model_name_or_path, ignore_patterns=BIN_EXTENSIONS
                )
...
class ActivationMixin:
    ...
    def get_activated_adapters(self):
        return [
            key for key, value in self._thread_inf.get(self.indent, {}).items() if value
        ]
            ...
            for name, _module in module.named_modules():
                ...
                for sub_name, param in _module.named_parameters():
                    ...
                    params[sub_name] = param_cls(
                        state_dict[prefix + sub_name], requires_grad=param.requires_grad
                    )
...
class SwiftAdapter:
    ...
    @staticmethod
    def prepare_model(
        model: torch.nn.Module, config: SwiftConfig, adapter_name: str
    ) -> SwiftOutput:
    ...
    def offload(module: torch.nn.Module, adapter_name, module_key, offload: str):
        ...
        if offload == "cpu":
            ...
            if str(device) != "meta":
                ...
                SwiftAdapter.offload_helper.offload_disk(
    ...
    def load(module: torch.nn.Module, adapter_name, module_key):
        ...
        if str(device) == "cpu":
            ...
            SwiftAdapter.offload_helper.load_disk(
                module, adapter_name=adapter_name, module_key=module_key
            )
            module.to(module.origin_device)
            delattr(module, "origin_device")
...
class ModulesToSaveWrapper(ActivationMixin, _ModulesToSaveWrapper):
    ...
    def set_adapter(self, adapter_name: str, offload: str = None):
        ...
        SwiftAdapter.save_memory(
            self.modules_to_save[adapter_name], adapter_name, self.module_key, True
        )
        SwiftAdapter.save_memory(
            self.original_module,
            "original_module",
    ...
    def deactivate_adapter(self, adapter_name: str, offload: str = None):
        ...
        if not self.get_activated_adapters():
            SwiftAdapter.save_memory(
                self.original_module, "original_module", self.module_key, True
            )
...
def swift_to_peft_format(ckpt_dir: str, output_dir: str) -> str:
    if "default" in os.listdir(ckpt_dir):  # swift_backend
        ...
        ckpt_dir = output_dir
        logger.info(
            f"Converting the swift format checkpoint to peft format, and saving it to: `{output_dir}`"
        )
    else:
