...
def prepare_markdown_item(item, markdown_prompts=None):
    ...
    if column_header and isinstance(column_header[0], list):
        ...
        columns = [
            f"{col[0]} {col[1]}" if len(col) > 1 else col[0] for col in column_header
        ]
    else:
...
def process_aitqa_data_batch(
    ...
    logger.info(
        f"Started processing AI-TQA data: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    logger.info(
    ...
    all_items = [
        (i, prepare_markdown_item(item, COT_PROMPT_AITQA_TEMPLATE))
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Process AI-TQA dataset with batch inference"
    )

    parser.add_argument(
        "--input_file",
        type=str,
        help="Path to input file (defaults to test.jsonl in data/aitqa)",
    )
    parser.add_argument(
...
def main():
    ...
    input_file = (
        args.input_file
        if args.input_file
        else os.path.join(base_path, "data/aitqa/test.jsonl")
    )
