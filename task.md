...
| RM-188 | Arch | Apple 风格 6-beat 叙事系统 — 替代 Duarte Sparkline 7-beat 成为叙事主干 | Done |
| RM-189 | Quality | 质量门控回归修复 — SVG sanitizer/hook verdict/data accuracy/background variety 一致性 | Done |
| RM-190 | Feature | Pseudo-3D SVG v1 — `svg-3d` element with layered depth, perspective tilt, parallax, float, shadow, reveal | Done | 新增 `svg-3d` 元素类型 (第 19 个原子元素)。Svg3dElement 渲染器: depthPreset (subtle/card-stack/exploded) 确定性层深度表, cameraTilt (left/right/top) wrapper 透视倾斜, parallax (none/subtle/medium) 逐层水平漂移, float 浮动动画, shadow (soft/medium/strong) CSS filter, reveal (fade/rise/draw)。共享 SVG sanitize 模块 (svgSanitize.ts) 修复 root `<svg>` 属性未清洁的安全问题。SVG Model 设置: Settings 新增 svgModel 下拉框 (默认 gemini-3.1-pro-preview), Multi-Agent Visual Director phase 检测 storyboard 含 svg-3d 时自动切换 Pro 模型。SVG 质量规则: elementCatalog 新增 svg_quality_rules (10 条通用 SVG 质量标准: 渐变/圆角/badge/连接线/最低 15 元素等), get_element_catalog tool 传递给 AI。3D Architecture test template 新增。新增 4 文件, 修改 15+ 文件, 13 sanitize tests + 8 svg-3d validate tests。 | 5 项修复: (1) foreignObject 从 ALLOWED_TAGS 正式移除 (安全优先, 测试改为验证剔除); (2) checkHookClaim title-card 豁免收紧: 新增 STRONG_VERDICT_PATTERN 排除 growth/leading 等主题词, number 检测改用 HARD_DATA_PATTERN 避免 Q4 误判; (3) checkDataAccuracy 归一化: 新增 canonicalize() 处理 $2B≡$2.0B≡2billion 等价形式, 去除 .0 尾数, 统一 magnitude suffix; (4) checkBackgroundVariety 补充 monotonous bgColor-only 检查 (≥5 scenes 纯 bgColor→fail); (5) 全量测试 429 pass, build 通过。修改 2 文件, 更新 2 测试文件。 | 核心改造: 叙事结构从 Hook→Context→Tension→Evidence→Climax→Resolution→Close 升级为 Apple 6-beat: Hook→Why It Matters→How It Works→Proof→Climax→Resolution。(1) video.ts: 新增 AppleBeat/LegacySceneRole 类型, StoryboardPlan 加 audienceMode/storyMode/coreTakeaway/hookStatement, StoryboardScenePlan 加 beat 字段; (2) promptStoryboard.ts: 完全重写 — 6-beat 合约+tone ladder (default/elevated/launch)+narration 句子纪律+scene count 6-9; (3) promptVisualDirector.ts: 新增 Visual Grammar — 每个 beat 的视觉映射 (hook→center/single focal, climax→spotlight+dramatic), hero element 上限 1, content element 上限 3, background rhythm 支持弧线; (4) evaluate.ts: 新增第 8 项 Apple Narrative Discipline 检查 (2 秒测试/单一信息/climax 强度/compressed close); (5) promptAgents.ts: parseScenePlan 支持新 beats+legacy→beat 映射, handoff 格式含新字段; (6) agentTools.ts: draft_storyboard 新增 audience_mode/core_takeaway/hook_statement 参数, direct_visuals scene_role 枚举更新。27 个新测试全通过, tsc 零错误。修改 6 文件, 新增 1 测试文件。 |

| RM-195 | Fix | SVG height sizing + bar chart label truncation + data accuracy false positives | Done | svgSanitize height="auto"→height="100%"+preserveAspectRatio. BAR_CHAR_WIDTH 0.55→0.65. coreDigitsMatch fallback for unformatted user data ("117,069.39"↔"$117K"). 3 new tests. |
| RM-196 | Fix | Annotation removed from quality gate gaming + enforce row layout | Done | annotation 从 PERSONALITY_TYPES/RICH_VISUAL_TYPES 移除。新增 annotation layout 检查 (非 row → 警告)。promptBlocks/promptReviewer/agentTools 同步更新。 |
| RM-197 | Feature | SVG post-generation architecture — decouple from produce_script | Done | 新建 svgGen.ts: 聚焦 Gemini 调用生成高质量 SVG (50+ visual elements, 之前 3-5)。generateScript.ts 6 阶段 pipeline。AI 输出 svgPrompt 描述, 后置生成填充 markup。SVG 无背景/标题/网格 (透明融入场景)。generateScriptHelpers.ts 提取。 |
| RM-198 | Feature | Real-time cost tracking from Gemini API usageMetadata | Done | costTracker.ts: 定价表 + 6 类别累加器。gemini.ts 提取 usageMetadata + costCategory 自动记录。所有调用点标记: agentPhase(agent), svgGen(svgGen), tts(tts), bgm+imageGen 固定费率。Console 输出 [Cost] 行。 |
| RM-199 | Fix | Cost UI badge + has_data detection + SVG scene title requirement | Done | Preview 标题栏 $X.XX badge (hover 明细)。has_data 改为数数字个数 (≥3 有意义数字 = 有数据)。SVG 场景必须有标题文字元素。 |
| RM-200 | Feature | Cost modal + sidebar + history cost tracking | Done | CostModal: 每类别横条图 + token 数 + 累计历史总额。Sidebar "Cost" 按钮。HistoryEntry 存 costUsd + costBreakdown。saveToHistory 接受 cost 参数。DividerElement NaN width 修复。Double-counting 修复。CSS overlay 修复。 |
| RM-201 | Fix | Restore TTS/BGM/image blobs from IndexedDB cache on refresh + history restore | Done | 根因: cache restore 只加载 script JSON, 不重新关联 blob URLs → 每次刷新白白重新生成 TTS (~$0.02)。修复: restoreTTSAudio + restoreBGMAudio + restoreImageBlobs 在 loadScript 后调用。History restore 先尝试 cache, 只对缺失 scenes 调 API。Cost 持久化到 localStorage。 |
| RM-202 | Fix | WebCodecs export uses waitFrame instead of waitMicrotask | Done | TTS/视频不同步根因: WebCodecs 路径用 waitMicrotask (setTimeout(0), ~0-4ms), React 还没渲染完就截帧 → 视频看起来比音频快。改用 waitFrame (requestAnimationFrame, ~16ms) 确保每帧渲染完成。 |
...
| Phase 4 | Pseudo-3D SVG elements (isometric diagrams, layered cards, parallax SVG scenes) | ✅ Done (RM-190) |
